...
| 5.6 | **Fuel Identity Refresh** | Existing Fuel screens into the same product family — presentation only, **not an architecture rewrite** | ✅ **COMPLETE / LOCKED** — founder-approved across 5.6A, 5.6A.1, 5.6B, 5.6C, 5.6D and 5.6E. **The whole Fuel experience is locked.** |
...
| 5.6E | **Fuel End-to-End Audit** | The final Fuel quality gate — the whole feature audited as one connected experience; two defects fixed, nothing redesigned | ✅ Approved — founder device review, 2026-09-13. **Closes 5.6.** |
...
**Quick Tools is now the user's.** `quickTools.ts` mirrors `modules.ts` exactly: a presentation-only registry, an untrusted-input normaliser, and pure `toggleTool` / `moveTool` / `visibleTools`. Order and visibility persist under their own key, `vita:v1:dashboard:tools`, **separate from the layout** so one write can never drop the other's state; `usePersistedPrefs` is the shared hook both now use. Hiding every tool removes the section entirely rather than leaving a heading over a void. The nested configuration sits **below** the module list in Customize Home rather than expanding a row, which keeps every draggable row exactly `ROW_HEIGHT` tall and the 5.3B drag arithmetic exact.

**The Food Scanner is back, by founder ruling — reversing 5.3B.** `Scan` routes to `/fuel/scan`, and the test that asserted Home must never navigate there is reversed with the reason recorded in place. **The part of the old rule that survives is the one about claims:** the tile opens the barcode scanner that looks a product up so it can be logged, and its spoken hint says exactly that. No VITA Score is authorised, so nothing in the label or hint implies a score, a grade or a rating — two tests hold that line, one over the registry and one over the rendered screen. Anyone who used Home before the tile existed gets it appended by `normalizeQuickTools`, with no migration step.

**The quote reads as a quotation.** A classical serif, italic, with an em-dash attribution on its own line — the only place on Home where the type changes, which is what separates it from app copy. **No font ships.** The audit found `expo-font` configured in `app.json` but no bundled files and no `fontFamily` anywhere, so iOS's built-in `Hoefler Text` was already available; Android takes its generic `serif` and everything else the system face.

**The greeting takes the hour's colour** — gold, amber, dusk violet, night indigo. Evening and night are muted derivatives rather than `palette.peptide` and `palette.water`, which at full saturation would read as feature accents sitting nowhere near their features. The colour decorates words that already say the time of day.

**Both accents are scheme-aware, and that is a defect the device pass caught.** `palette.gold` is drawn for dark surfaces; on the cream background it sits at roughly **1.7:1**, well under the 3:1 WCAG asks of large text, and looked washed out on device. Light mode takes deepened versions of the same four hues rather than dropping the colour.

**Home is rearranged on Home.** Hold a widget: the grid enters edit mode, widgets jiggle under half a degree, each grows a remove control, and one can be dragged onto another to trade places. `Done` replaces the header's two icons while the mode is active — a gesture-entered mode needs a visible, labelled way out, because tapping empty space is neither discoverable nor reachable by voice.

**The long press is deliberately wired in two places.** React Native hands the responder to the *innermost* pressable, so a hold landing on Water's own square is Water's to handle and a wrapper above it would never see the gesture; `onLongPress` is therefore threaded into the three feature modules, where `Pressable` also suppresses the tap on release so holding a widget cannot both rearrange Home and open the feature. A wrapper catches the rest. **The obvious-looking alternative — one transparent overlay per widget — cannot work:** an overlay that can receive a hold receives every tap, so Add, Log and the tool tiles would all stop responding. While editing, a blocking layer *is* correct and is used, so a grab cannot fire a module's own button.

**Drop-on-release, not live reflow.** Reordering continuously mid-drag would mean recomputing the grid, re-measuring every cell and compensating the gesture baseline on every crossing — several chances to get a swap subtly wrong in an environment that cannot test the gesture by hand. Dropping is one decision from one final position. Each widget measures itself with `measureInWindow` and **re-measures when edit mode is entered**, so every rectangle comes from one frame at one scroll offset; because the drag delta is applied to the dragged widget's own stored rectangle, the comparison is scroll-invariant. A drop that lands on nothing leaves the order alone.

**No new dependency, again.** No gesture library, no animation library, no font. `PanResponder` and `Animated` are React Native core; the diff to `package.json` is empty. **Customize Home remains the complete, accessible surface** — a hold is unavailable to someone using VoiceOver or a switch, and hiding, resizing and reordering all stay reachable without a pointer. The remove control says *Remove … from Home*, never *Delete*: nothing is destroyed and Customize Home offers it straight back.

**Validation.** `npm test` **51 suites / 1341 tests** (1309 → 1341) · `tsc --noEmit` clean · `--noUnusedLocals --noUnusedParameters` clean · `expo install --check` unchanged (the two Expo patch drifts predate this slice) · iOS export clean · **no dependency added** · `git status` confined to `features/dashboard/` and its route, zero `src/lib` changes · verified in Expo Go: Dark and Light.

**Device coverage limit, stated plainly.** The Simulator MCP refuses with an Xcode-configuration error that is spurious — `xcode-select -p` already points at `/Applications/Xcode.app/Contents/Developer` — so this environment can deep-link and screenshot but **cannot tap or long-press**. Edit mode, the jiggle, the drag and the drop were therefore verified by 20 route tests and by reading the responder wiring, **not on device**. Two real defects the screenshots *did* catch — the Water square overflow and the light-mode gold contrast — are both fixed above, which is the argument for having run the pass at all.
...
Fuel was never in Sprint 5's original plan. Sprint 2 built its functionality and its presentation predated the current identity, so the founder's 2026-09-04 ruling gave it a dedicated slice. It closed across six passes:

| Pass | What it settled | Status |
|---|---|---|
| **5.6A** | A real Fuel test baseline, and the end of invented nutrition goals | ✅ |
| **5.6A.1** | Goals narrowed to calories and protein; carbs and fat became totals | ✅ |
| **5.6B** | Fuel Home Identity, across five passes | ✅ Locked 2026-09-09 |
| **5.6C** | Add Food, Search, Recent, Favorites | ✅ Locked 2026-09-09 |
| **5.6D** | Scanner, Food Detail, Manual Entry, Edit Entry | ✅ Locked 2026-09-13 |
| **5.6E** | The end-to-end audit and final quality gate | ✅ Locked 2026-09-13 |

**Tests across the whole refresh: 64 suites / ~1,700 at the start of 5.6A to 84 suites / 2,224 at the close.** Fuel had **zero** tests when 5.6 opened — not one of the 64 suites asserted anything about `lib/nutrition`, `features/fuel` or any `/fuel/*` route.

---

#### The final locked Fuel product

**Fuel Home.** Header → Nutrition → Meals → Water + Peptides → Add Food. The Day Strip is implemented, optional and **hidden by default**. The header carries Fuel's identity, the date chip, the scanner, Customize Fuel and Settings.

**Nutrition.** Calories: the consumed amount leads, with an optional **user-authored** goal, what is left or what is over, and a slim rail — all three present only against a real goal. Protein: a factual total, with an optional user-authored goal and a rail only when one exists. **Carbs and fat: factual totals only.** No carb target, no fat target, no recommendation, no score.

**Meals.** Breakfast, Lunch, Dinner, Snacks — the same time-of-day identity on every surface, compact, collapsible, on real logged food.

**Water.** Shared with Dashboard: one canonical Water state, goal, unit and vessel semantics. **No duplicated Fuel water state.**

**Peptides.** One shared compact presentation with Dashboard — read-only factual awareness.

**Add Food.** Search-first, never a menu. Idle shows Recent and Favorites; Scan and Manual are secondary.

**Search / Recent / Favorites.** One food-row language, one `foodVisual` resolver.

**Scanner.** Barcode acquisition only. No health scoring.

**Food Detail.** The founder-approved direct-on-background hierarchy: food identity, serving, calories for the selected serving, Protein / Carbs / Fat, amount, meal, and the neutral logging action.

**Manual Entry.** The same product language as Food Detail, with inline validation. An invalid Save shows errors and **writes nothing**.

**Edit Entry.** Historical snapshot safety preserved — editing one entry never mutates the catalog food, another entry, or any other snapshot.

---

#### Standing Fuel invariants

These survive the closeout and bind future slices:

- **Cross-feature consistency.** Equivalent state uses equivalent meaning and presentation language.
- **One meal identity** — `mealAccent` — for Breakfast, Lunch, Dinner and Snacks across every Fuel surface.
- **One macro identity** — `macroAccent` — for Protein, Carbs and Fat. **Category identity only, never a health verdict.**
- **One food visual** — a real image, then the `foodArt` fallback.
- **One calorie derivation** — `calorieSummary` — shared by Fuel Home and Dashboard.
- **One Water state** behind Fuel, Dashboard and the Water feature.

**What Fuel deliberately does not do.** It does not recommend calories, protein, macros, foods or meal plans; does not calculate TDEE or BMR; does not grade foods, assign a VITA Score, score scanned products, or score ingredients.

---

#### What 5.6E found, recorded accurately

**Corrected:**

1. Logged food names inside expanded Fuel meals could crop, from the old one-line cap.
2. The user-facing calorie-goal editors said `kcal` where VITA's product language says `cal`.

**Identified and deliberately not changed:** multiple founder-approved primary-action geometries across the wider product · the known shared `ScreenHeader` truncation at accessibility-extra-large · the existing Expo patch drift. All three remain open in `docs/07-Audit-Log.md`.

---

**Next product slice: 5.7 — Tools + Settings Identity Integration. Not started.** Expo SDK 57 patch maintenance is taken first, on its own branch.

### Slice 5.6E — Fuel End-to-End Audit — closed ✅

**Founder-approved on a physical iPhone, 2026-09-13. Approved implementation: `c994cfe — feat(sprint-5): polish fuel end-to-end`.**

The founder reviewed the final Fuel flow, checked the 5.6E findings specifically — including the expanded-meal long-food-name behaviour — and approved the resulting experience. **This approval closes the whole of 5.6; see *Slice 5.6 — Fuel Identity Refresh, closed* below.**

The final Fuel quality gate: the whole feature audited as one connected experience rather than as a list of screens. **Not a redesign.** Every component had already passed founder device QA, so the brief was to preserve what works and change only what is demonstrably wrong.
...
**Answered by the founder's device review.** A long food name inside an expanded meal reads in full and the row grows cleanly — the fix this environment could not see is confirmed on hardware. The founder approved the Fuel experience as it stands, which leaves the bare calorie figure inside a meal and the Setup pill as they are; both stay recorded in the Audit Log as open product questions rather than defects, and neither blocks the Fuel closeout. The nine end-to-end flows were exercised by touch.

### Slice 5.6D — Fuel Logging Flow — closed ✅

**Founder-approved on a physical iPhone, 2026-09-13. The scanner, Food Detail, Manual Entry and Edit Entry are locked.** Approved implementation: `cf53a01 — feat(sprint-5): unify fuel logging flow`.

The founder verified on device: the scanner and real camera behaviour · the barcode flow · Food Detail · Manual Entry · Edit Entry · meal-context continuity · macro visual consistency · food visual consistency · serving and quantity behaviour · logging · the current Fuel visual language.

**The approved Food Detail hierarchy, as reviewed.** This exact direction is approved and is not to be redesigned without a demonstrated defect:

```
Dinner                        ← shared MealContext, mealAccent
Greek yogurt                  ← shared FoodIdentity, foodVisual resolver
Fage · 1 container (170 g)
1 CONTAINER (170 G)           ← the portion these figures describe
140  Calories                 ← this serving, never "Calories consumed"
PROTEIN   CARBS   FAT         ← macroAccent; accent on the label only
18 g      6 g     4 g         ← values neutral and high-contrast
AMOUNT
1 container (170 g)  − 1 +
MEAL
Breakfast  Lunch  Dinner  Snacks
Add to Dinner                 ← neutral primary, names its destination
```

**The locked product invariants.** Not restyleable by a later slice:

- **The scanner is barcode acquisition, and only that.** Permission, capture, normalization to GTIN-14, lookup, duplicate-scan suppression, success into the shared Food Detail, not-found, lookup failure, retry, Search and Manual fallbacks, and meal-context preservation. It does **not** score, grade, rate, recommend alternatives or analyse ingredient risk. No VITA Score, and it is not becoming Yuka.
- **One macro identity**, `macroAccent`: protein a restrained dusk violet, carbs warm amber, fat cool steel blue, values neutral. Colour means **category**, never good, bad, healthy, unhealthy, warning, success, over or under. No Fuel screen may choose another mapping.
- **One meal identity**, `mealAccent`: Breakfast sunrise, Lunch sun, **Dinner moon**, Snacks utensils — the same glyph, accent and semantics on Fuel Home, Add Food, the scanner, Food Detail, Manual Entry and Edit Entry. No local duplicates.
- **One food visual**, `foodVisual` via `FoodAvatar`: a usable real image, then VITA's `foodArt`. No separate detail or scanner placeholder system.
- **Manual Entry is the manual equivalent of Food Detail** — task header, shared meal context, direct grouped Food / Serving / Nutrition, the same macro identity, `TextField`/`NumericField` conventions, concise inline validation, a neutral Save, and no schema jargon.
- **Manual validation, explicitly approved.** Save may stay enabled on an incomplete form. Pressed with invalid or missing data it shows inline errors, **persists nothing**, and does not navigate as though it succeeded. The safety invariant is that invalid data never reaches storage — *not* that the button is disabled. This is not to be reverted because an older characterization test expected a disabled control.
- **Edit Entry is the same visual family**, loads the `FoodEntry` snapshot, edits amount, serving and meal, saves, and deletes. Editing one entry never mutates the provider or catalog `VitaFood`, another historical entry, or any other snapshot.
- **The primary action is neutral** and names its destination. No return to a full-width orange slab; Fuel orange is an earned accent.
- **`TextField` labels are programmatically connected to their inputs** — a shared accessibility correction made in 5.6D, not to be reverted.

**Data safety, confirmed at lock.** 5.6D changed no `VitaFood` schema, no `FoodEntry` schema, no food-log architecture, no OFF or USDA adapter, no ranking, dedupe or search cache, no Recent or Favorites semantics, no nutrition goals, and nothing in Water, Peptides, Dashboard, Fuel Home or the Add/Search presentation. **No persistence key was added.**

The screens *after* a food is chosen were brought into the language Fuel Home and Add Food locked in. **No Fuel Home change, no Dashboard change, no Water or Peptides change, and no change to Add Food, Search, Recent or Favorites** — those are locked, and the diff proves it.
...
**Answered by the founder's device review.** The flow from Fuel Home through Add Food to Food Detail reads as one continuous feature; Dinner's moon is the same mark on all five screens; the macro colours are the same three on Fuel Home, Food Detail and Manual Entry; a real barcode resolves through the real camera into the shared Food Detail; and Manual Entry's numeric Done bar behaves. The founder exercised the camera, the keyboard and the touch targets this environment cannot.
...
**One component each, one derivation each, one footprint.**

- `components/modules/CompactWaterModule` and `CompactPeptidesModule` — strictly presentational. Each takes a view model and callbacks, imports no domain, and calls no data hook.
- `compactWaterView` in `lib/water` and `compactPeptidesView` in `lib/peptides` — the copy rules, in the domain that owns them, tested there. **Home's wording is the source** in both, per the founder's ruling that where Home already has the approved presentation, Fuel converges to it.
- `components/modules/geometry.ts` — the square height, the two radii, the Dynamic Type threshold and the module type scale. `features/dashboard/widget.ts` re-exports every name, so Home's own components import exactly what they always did.
- Home's `WaterModule` and `PeptidesModule` are now ten-line adapters over the shared pair; Fuel's `FuelModules.tsx` is deleted.

A test mounts **both routes over the same storage and asserts the two `view` props are deeply equal** — located by component type, so a screen that ever goes back to drawing its own fails it.

**The ring became VITA's fillable vessel.** Founder ruling: Water owns a distinctive object from 5.2 and the compact modules were drawing the one shape every health app has. The same `WaterVessel` now appears at three sizes — the Water screen's 116pt hero, a 38pt square module and a 22pt slim form in the wide strip. **No second bottle was drawn and no second hydration calculation exists**: it is the same component fed `progress` from Water's own arithmetic, in Water's own display unit. No goal hands it `null`, so it draws latent rather than at a false 0%.

**The Day Strip is hidden by default — and not deleted.** The founder's reversal after seeing Nutrition, the strip and four meal rows together: the strip added density without adding an answer, and crowds on a busy day. It remains a full section in Customize Fuel and behaves exactly as before once switched on. **Default order is now Nutrition → Meals → Water → Peptides**, with the strip last and off.

**A changed default must not overwrite a made choice.** `normalizeFuelLayout` now distinguishes the two: a stored record carrying a `hidden` array was written by someone using Customize Fuel and is honoured exactly — **including an empty one, which means *show everything*** — while a record with no `hidden` field at all (every 5.6B.1 layout, since it stored a bare order) takes the new default. *Reset Layout* is the deliberate way back.

**Nutrition typography.** The calorie figure keeps its size and gains a real line height and tighter tracking, replacing the negative margin that had been standing in for one — a hack that does not scale with Dynamic Type. Macro values rose to 17pt, the same size the shared modules give their value line. **Protein's rail is no longer green**: the macro tokens are green, amber and red, and a green bar under protein is the same traffic-light reading that got the composition bar deleted. It is drawn in secondary text instead — calories carry the feature colour because they are the subject; protein's rail is secondary progress. Carbs and fat hold the rail's height as **empty space, never a track**, so the three columns end level.

**Two defects the device pass caught and fixed.** The vessel's stroke widths were authored in viewBox units and shrank with the drawing, so at 38pt the outline rendered at under half a pixel and the module read as a dark slab; strokes are now stated in points and converted. And Fuel's square adopted Home's 208pt footprint, which resolves 5.6B.2's `fl oz` clipping properly — the cap was never the cause, the 168pt base was.

**Dead code removed.** `usePeptideSummary`, whose last caller was Fuel's old module and which answered a different question from the canonical selector; the `hasGoal` field from the water view, which restated what `progress === null` already says. `ProgressRing` now has no caller in `src/app` or `src/features`; it is left in place as a founder-approved shared primitive rather than deleted in a polish slice.

**Boundaries unchanged.** No carb or fat target, ceiling or limit. No calorie, protein or macro recommendation, no TDEE, no BMR, no macro split. No VITA Score, no food grading, no composition bar. No peptide dose, protocol or titration, and the compact module has nothing to write with. No Fuel quote.

**Validation.** `npm test` **75 suites / 1974 tests** (1947 → 1974) · `tsc --noEmit` clean · `--noUnusedLocals --noUnusedParameters` clean · iOS export clean · **no dependency added** · no persistence key added or removed · verified on device in Dark, Light and accessibility-large, on **both Home and Fuel**.

**Known maintenance, carried forward unchanged.** `expo install --check` and `expo-doctor` (**20/21**) both report `expo 57.0.20` and `expo-router 57.0.19` one patch behind. This authorization explicitly excluded upgrading them; it is a separate maintenance action for after Fuel Home locks.
...
**Five sections, all direct on the background.** Day Strip · Nutrition · Meals · Water · Peptides. No cards, no rings, no filled orange blocks, no mega-container — the structure came back, the bulk did not.

**Meals are four compact collapsible lines.** All four slots are present again (5.6B hid the empty ones, which took the orientation away with the weight), each one line: name, subtotal and food count, or *No foods logged* and a `+`. **Collapsed by default**, because the Day Strip above already answers *what did I eat* and this section answers *what was in one meal* — a question asked one meal at a time. Session-local state; which meal someone folded open is not worth carrying across launches.

**Water and Peptides are back, as sections rather than tiles.** The old versions were bordered cards with a progress bar that had nothing to fill. These are headings with one line of real state and one action — Water shows its own goal and progress with a blue rail, Peptides reports what is scheduled with a way through. **Fuel only reads peptide data**, and neither embeds its feature's screen.

**Set up Fuel replaces the quiet goal line.** A user who has configured nothing — no calorie goal, no protein goal, no water goal — is invited once into `/fuel/setup`: three optional fields, Save, and a real *Skip for now*. Skipping is remembered, so the invitation does not return on every launch, and a small *Set up Fuel* action stays reachable afterwards. Setting any goal removes it on its own. **Fuel logs food, counts calories and scans barcodes with no goals at all.**

**One water goal, and it is Water's.** The setup surface writes calories and protein through `updateTargets` and the water goal through Water's own `setGoal` — the same function the Water screen's editor calls. Setting 8 cups in Fuel is setting 8 cups in Water, immediately, and Home follows because all three read the same provider. Fuel converts nothing and stores nothing of its own; the figure renders in the user's Water display unit, so a goal saved as 8 cups reads as 64 fl oz for someone set to ounces. Four tests pin it, including one asserting no Fuel-shaped water key is ever written.

**Sections can be reordered.** Hold any one to arrange Fuel: an outline, a handle, **Move up / Move down** buttons, and a vertical drag, with Done in the header. **Ordering only** — no sizes, no hiding, no delete `×` — because Fuel is one workflow whose hierarchy should be right by default, and the only thing worth varying is which part someone reads first. The drag and the buttons resolve through the same helpers, so they cannot disagree. Persisted at `vita:v1:fuel:layout`, normalized on read against unknown, duplicate and missing sections. Header, date, scanner, settings and Add Food are not sections and do not move.

**Two things the device pass caught and fixed.** The meal shortcut chips beneath Add Food now duplicated the four meal rows that had just come back — removed. And the resting Day Strip opened a large void between the header and the day — tightened.

**`usePersistedPrefs` moved to `src/lib/preferences/`.** Fuel needs the same persisted-preference hook Dashboard uses, and features never import each other; promoting shared code to `src/lib` is what rule 4 prescribes. Dashboard's own behaviour is unchanged and its suite passes.

**Boundaries unchanged.** No carb or fat target and no "do not exceed" limit — VITA has no validated basis for one. No calorie, protein or macro recommendation, no TDEE, no BMR, no macro split. No VITA Score and no food grading. The composition bar stays removed. A future Journey may one day assist with nutrition guidance; **nothing of the sort exists and none is implied.**
...
**Setting a routine up is now a sequence: prepare it, then track it.** New Setup opens on **Preparation** with one question — *Set up vial* or *Already prepared* — and Routine, More options and *Add to Routine* follow. **Editing is not a sequence and was not reordered**: nobody opens a running routine to reconsider how their vial arrives, so Edit still opens on Routine with Preparation collapsed beneath it. Same fields, same validation, same emit path in both; only the order and the framing differ.

**"Already prepared" is a valid answer, not a skipped step.** Someone whose clinic hands them a prepared pen has not omitted anything, and "Skip preparation" would have framed a complete situation as an omission. Choosing it **stores nothing** — no vial amount inferred, no reconstitution defaulted, no conversion shown, because with no concentration there is no honest conversion — and it *clears* anything typed before the choice changed, since a value the form has stopped showing is a value the user cannot correct. Preparation remains optional either way: leaving the question unanswered saves a perfectly valid routine.

**The calculator is behind a disclosure, summarised by the one line people open it for** — `1 mg = 20 units`, built from the same reference the table is, so the two cannot disagree. The full table, the concentration, the U-100 assumption and the custom conversion are unchanged, one tap in. A setup screen should not read as an engineering tool.

**The peptide says what it is, in the catalog's own words.** `Semax / Research / ACTH (4-10) Analog` was three facts stacked in descending order of usefulness; the class now sits directly under the name with the regulatory badge after it. `peptideDescriptor` reads the existing reviewed `category` field and adds the compound type only when it is *not* a peptide — the genuinely useful fact in a peptide tracker. **The §24 audit answer: no entry lacks a descriptor.** All 96 built-in entries carry a `category`, the longest is 39 characters, and this slice authored **no new catalog content at all**. A content test runs the boundary over the whole catalog rather than a fixture.

**The number pad's Done key is now an input-system behaviour, not a screen's problem.** It was opt-in: `NumericField` pointed at a shared `nativeID` that each screen had to remember to register, and the failure was silent in both directions. Founder QA found it three times in three slices — the calculator in 5.2, the `TakenSheet` in 5.5, a Water field in 5.5C — and each was patched where it was found. **The bar now lives inside the primitive**, one per field with its own generated id, which also makes it modal-safe by construction: an `InputAccessoryView` must share the presented hierarchy of the input it serves, and a field that carries its own accessory is always in the right one. The audit found **eleven inputs with no Done key at all**: nine in Fuel's manual entry, the Water goal, and the Water custom amount editor. All eleven are fixed, no screen renders an accessory any more, and a source-scanning test fails the build if any file outside the primitive names a number pad or mounts a bar of its own.

**Month summary is readable without squinting.** Title, labels and counts each one step larger in a taller row, with the label at full text colour; the state marker stays a 6pt dot, because the numbers are the content and hero metrics would make a quiet record look like a scoreboard. Extracted to a shared `MonthSummary` so the routine month and the new all-routines month cannot drift apart.

**A month across every routine, at `/peptides/activity`.** Home answers *what is scheduled today*; a routine's calendar answers *how has this one gone*; neither answered *what did I actually do in July*. Reached by one restrained row after the routines on Peptides Home — a row, not a card, with no count on it, because the depth is behind it and a statistic here would be a statistic on a screen that deliberately has none. **A day is never flattened into one state**: two taken and one skipped draws three small marks, never a blended square or a pie chart, and selecting the day lists what actually happened by routine, from the stored records. Counts are **routine events**, not days — two routines taken on one day is two — and still only Taken · Skipped · No response.

**Device QA found one real defect in that calendar and it was fixed.** Three routines put three grey *no response* marks on each of twenty-four days still to come, so two thirds of September rendered as unanswered — a wall of failure for days nobody has had the chance to answer. The all-routines month now ends at today. Semantics are untouched — `markForDay` still decides every day, and the counts were always bounded by today — what changed is that the grid no longer draws an absence of an answer as though it were a missing one. The single-routine calendar still draws days ahead, where one hollow node per day reads as *scheduled* rather than as failure.

**§44, answered by looking:** Home already has a `Your routines` section, so Monthly activity is the only depth this slice adds. No tips, no motivational copy, no educational carousel, no streak, no stats card.

**§18 declined, deliberately.** Estimated doses remaining and vial duration were authorised as *optional*; the same section warns not to expand scope if it harms simplicity, and adding derived arithmetic to a screen whose whole purpose this slice was to shorten would have worked against it. Nothing blocks it later.

**Two small shared-primitive changes, both caused by this work.** `Button` gained an optional `accessibilityLabel` — it had none, and prior slices worked around that by swapping to `PressableScale`, which is the worse pattern. The custom injection-site confirm used it: it was a bare "Done" with no spoken label, which collided with the keyboard bar once every numeric field carried one. Its sibling confirm already followed the rule; this one had been missed.

**No-recommendation boundary, re-asserted.** No popular, common, typical, starting, beginner or recommended dose; no titration, no escalation table, no protocol, no preset dosing buttons, no "most users take", no suggested amount, no schedule recommendation, no adherence grade. Tested over the setup screen, the calculator and the whole catalog rather than assumed.

**Validation.** `npm test` **63 suites / 1634 tests** (1555 → 1634) · `tsc --noEmit` clean · `--noUnusedLocals --noUnusedParameters` clean · `expo install --check` up to date · `expo-doctor` **21/21** · iOS export clean · **no dependency added**, `package.json` diff empty · **zero changes under `src/lib`** · no persistence key, schema or migration touched · verified in Expo Go: Dark, Light, and `accessibility-extra-large`.

**Removed:** `earliestKnownMonth`, which bounded navigation in 5.5A and has bounded nothing since 5.5B replaced it with a real storage read. Leaving it invited someone to wire it back.

**Device coverage limit, unchanged.** This environment deep-links and screenshots but cannot tap, so selection, the preparation choice and the calculator disclosure are covered by route tests and read code rather than by finger. `ScreenHeader` still truncates at accessibility sizes — a known deferred item (§76), visible in the Dynamic Type screenshots and untouched here.
...
**The remove control moved to the top-right corner**, straddling it, on all five removable modules. That is where every home screen puts it, and the corner is the one part of a VITA widget that carries no content — the feature label sits top-left and the action runs along the bottom, so nothing is covered. 28pt of badge plus 9pt of slop clears the 44pt minimum without drawing a control that large over the widget. Still *Remove … from Home*, never *Delete*: a test confirms the day's real Water entries and goal survive hiding the widget.

**The drag is live.** A widget lifts (1.03, a soft shadow, raised above its row), follows the finger, and its neighbours glide toward the positions the candidate order would give them **while the finger is still down**. Release settles it into the slot it is already over.

**Nothing about the layout changes mid-drag, and that is what makes it safe.** The rendered order is frozen for the whole gesture and every widget is *translated* instead — re-rendering the grid would move the dragged widget's own cell out from under the gesture, which is exactly why 5.3C deferred to release. `dragLayout.ts` holds the arithmetic, pure and free of React: where each widget would sit under a candidate order, which slot a point has meaningfully entered, and how far everything else must move. **19 tests cover it directly**, which is how a gesture this environment cannot perform gets verified at all.

**The commit happens on release, before the settle animation, and it is invisible.** Every other widget is already sitting at its new slot, so zeroing its offset changes nothing on screen. The carried widget is the only one still elsewhere, so its offset is re-expressed against its *new* slot — same pixel, different frame of reference — and animated to zero. Committing before the animation rather than in its completion callback means an interrupted or dropped frame can never leave Home showing one order and remembering another.

**A fifth of a cell on every edge before a swap fires.** Without that inset a hand resting on a boundary reorders Home continuously. The test uses the hysteresis that falls out of it: once the swap happens the carried module owns that slot, so holding still proposes nothing and the screen stops shuffling. **Still no stored coordinates** — order, visibility and size are all that persist, and the grid derives placement.

**Reduce Motion gets the same outcome without the travel**: no jiggle, no reflow animation, no settle — widgets are placed immediately and the positional state stays completely clear. Normal motion is unaffected, which was the founders' explicit concern.

**Typography went up one step across Home** — greeting 11 → 12.5, quote 19 → 21, attribution 12 → 14, module labels 11 → 12.5, square values 17 → 20, support copy 13 → 14.5, wide values 15 → 17, actions 13 → 14.5, tool labels 13 → 15, sheet copy 13 → 14/16. The gaps between roles are preserved, so a value still dominates its label and nothing secondary was promoted; the sizes live in one documented `TYPE` map in `widget.ts`. **Kept local to Home on purpose** — `theme/tokens.ts` sizes the whole app, and starting a global typography migration inside a Dashboard polish slice is not what this slice is for.

**The attribution now belongs to the quote.** It was grey sans-serif body copy under a serif line, which read as two unrelated things; it now shares the quote's serif and italic, hangs right-aligned under the last line with a small inset, and takes a dimmed version of the quote's own gold. Spoken as one sentence — `I came, I saw, I conquered. Julius Caesar.` — because the em dash is typography, not something to read aloud. The shared square footprint rose 192 → 208 to carry the larger type.

**VITA now respects the device's text-size setting, and this slice is where that policy is set.** The audit found nothing in `src/` passing `allowFontScaling={false}` — a good baseline — and nothing here starts. The square footprint and tool tiles *scale with the text* rather than staying constant, damped, and still one shared value so Water, Peptides and Fuel remain equal at every size; past 1.25× the ring and the calorie bar step aside and hand their space to the words. Both are decorative and already hidden from screen readers, so nothing is lost, and **no figure is abbreviated and no spoken label changes** at any size.

**Information scales without limit; ornament does not.** The device pass at `accessibility-large` found the quote running to four lines and pushing the day's real figures off the screen, so the quote, its attribution and the wordmark carry a `maxFontSizeMultiplier` of 1.35. That is not `allowFontScaling={false}` — they still grow — and a test asserts those three are the *only* capped strings on the screen. The same pass found the wide Fuel strip truncating its own value to `2,000 c…`; a figure is information, so wide and square values now wrap rather than truncate once the text is large.

**Customize Home's rows scale too.** They were a fixed 64pt, which clips its own labels at an accessibility size. They are now uniform *but not constant* — uniform is all the sheet's drag arithmetic ever needed.

**Icon glyphs are the one exception**, and a deliberate one: `@expo/vector-icons` renders them as `Text` with scaling off. They are graphics sized in points, the library sets the flag itself, and the test excludes them explicitly rather than silently.

**Found and not fixed: the dock clips its own labels at accessibility text sizes** (`Journey` wraps to `Journe/y`). It is real, it is visible on Home, and it is **out of scope here** — §2 preserves dock styling and §47 forbids redesigning it, and it belongs to the app shell rather than to Dashboard. It should go to 5.7 with the `PressableScale` flex trap.

**Validation.** `npm test` **53 suites / 1396 tests** (1341 → 1396) · `tsc --noEmit` clean · `--noUnusedLocals --noUnusedParameters` clean · `expo install --check` unchanged (the two Expo patch drifts predate this sprint) · iOS export clean · **no dependency added** — no Reanimated, no Gesture Handler, no drag library; `PanResponder` and `Animated` are React Native core and the `package.json` diff is empty · diff confined to `features/dashboard/` and its route, zero changes under `src/lib`, `src/components` or `src/theme` · verified in Expo Go at Dark, Light, `extra-extra-extra-large` and `accessibility-large`.
