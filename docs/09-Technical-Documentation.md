...
**Nutrition goals are the user's, or absent (slices 5.6A / 5.6A.1).** `NutritionTargets` carries **calories and protein, both optional, and may be `null` entirely**, and `null` — never a default — is what a user has until they set one. Either may be set alone. Goals persist at `vita:v1:targets`; **Settings → Nutrition Goals** is the only writer, and Fuel Home shows a quiet *Set nutrition goals* line into it while the user has neither, so goals are discoverable from where they matter rather than only from Settings.

**Carbohydrate and fat are tracked totals with no target** (founder ruling, 5.6A.1). They are summed, stored and displayed exactly as before and can never gain a denominator: `Carbs 108 g`, never `Carbs 108 / 214 g`. `MacroKey` was the intersection of `NutritionFacts` and `NutritionTargets` keys, which only held while every macro had a target; the two ideas are decoupled now, and `macroHasGoal` names the one macro that can be measured. A record stored during 5.6A may still carry `carbs`/`fat` keys — `parseTargets` walks `GOAL_FIELDS`, so they are inert on read and dropped on the next save. No migration was written for unreleased development-state data.

Until 5.6A the type required all four macros and shipped a `DEFAULT_TARGETS` constant of 2,000 kcal / 160 g / 214 g / 64 g, carried over from two Sprint 1 fixtures. The provider substituted it whenever storage held nothing — which was always, because `updateTargets` had no caller — and every Fuel surface presented those figures as *the user's own targets*. They were never written to disk, so removing them needed no migration. **VITA does not invent nutrition goals**, and derives none from age, sex, height, weight, activity or a weight target: there is no TDEE, no BMR, no deficit and no macro-split calculator. **Deferred product direction:** a future Journey that knows a user's broader goal, starting state and progress may one day *assist* with nutrition guidance. Nothing of the sort exists, none is authorised, and no algorithm is implied by this note. `dailyTotals` returns `null` for `caloriesRemaining`, `caloriesOver`, `calorieProgress` and `caloriePercent` when there is no calorie goal, so a surface cannot render a percentage of nothing by accident.
