...
**Nutrition goals are the user's, or absent (slice 5.6A).** `NutritionTargets` has **four optional fields and may be `null` entirely**, and `null` — never a default — is what a user has until they set one. Any subset is valid: a calorie goal with no macro goals, or a protein goal alone. Goals persist at `vita:v1:targets` and are authored at **Settings → Nutrition Goals**, the only writer.

Until 5.6A the type required all four and shipped a `DEFAULT_TARGETS` constant of 2,000 kcal / 160 g / 214 g / 64 g, carried over from two Sprint 1 fixtures. The provider substituted it whenever storage held nothing — which was always, because `updateTargets` had no caller — and every Fuel surface presented those figures as *the user's own targets*. They were never written to disk, so removing them needed no migration. **VITA does not invent nutrition goals**, and derives none from age, sex, height, weight, activity or a weight target: there is no TDEE, no BMR, no deficit and no macro-split calculator. `dailyTotals` returns `null` for `caloriesRemaining`, `caloriesOver`, `calorieProgress` and `caloriePercent` when there is no calorie goal, so a surface cannot render a percentage of nothing by accident.
