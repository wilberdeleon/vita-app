...
**Current: Sprint 4 — Settings + Tools & Reference**, per the founder reorder of 2026-09-01, on branch `sprint-4-settings-tools-reference` cut from the merged `main`. **Authorized 2026-09-01** against its approved planning audit, with an eight-slice sequence; **slice 4.1 — Settings Foundation is implemented and awaiting founder device review.** Scope rulings recorded at authorization: [[BMI Calculator|BMI]] is committed, [[Research Library]] is architecture-only while [[Open Questions]] #17 is open, the [[Food & Product Scanner]]'s **scoring is not authorized**, and the Dashboard Tools shortcut is deferred.
