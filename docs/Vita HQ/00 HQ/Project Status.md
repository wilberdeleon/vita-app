...
**Sprint 4 is open and building.** Its planning and architecture audit was founder-reviewed and approved on 2026-09-01 and an eight-slice sequence authorized: Settings foundation → Tools & Reference hub → existing Tools polish → [[BMI Calculator|BMI]] → [[Research Library]] foundation → *(conditional, not promised)* scanner shared-data expansion → integration → audit. **Slice 4.1 — Settings Foundation is implemented and awaiting founder device review.** Two scope rulings: the [[Food & Product Scanner]]'s **scoring is not authorized**, and the Research Library ships as **architecture without content** while [[Open Questions]] #17 stays open. See [[Roadmap]], [[Current Sprint]], [[Decision Log]].
