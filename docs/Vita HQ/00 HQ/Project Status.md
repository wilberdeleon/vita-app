...
**Next: Sprint 5 — VITA Identity & Interaction.** The roadmap-alignment session ran on 2026-09-01 and inserted it ahead of Journey. Its objective is to **make VITA feel unmistakably like VITA** — establishing the visual and interaction language before Journey is built in it, because real-device review found the presentation layer had become too visually generic: dark background, large rounded card, text, icon, another rounded card, repeated across features that behave nothing alike.

**Sprint 5 is planned, not opened.** No branch, no approved slice, and **no redesign, BMI, scanner or Journey implementation.** Its draft slice structure (5.1–5.8) needs a Sprint 5 architecture audit and founder authorization first. Everything Sprint 4 built — Settings architecture, Appearance persistence, Units, `/tools`, the hub, the tools themselves, the Body Map primitives, domain logic, persistence and repositories — is **preserved and not rebuilt**; the sprint is primarily presentation and interaction work. **[[BMI Calculator|BMI]] remains planned and is not cancelled.** **[[My Journey|Journey]] moves to Sprint 6, deferred and intentionally delayed until the identity sprint is approved — not cancelled or reduced.** Brief: repo `docs/Sprint-5-Identity-Brief.md` (**DRAFT / PENDING SPRINT 5 ARCHITECTURE APPROVAL**). See [[Roadmap]], [[Current Sprint]], [[Decision Log]].
