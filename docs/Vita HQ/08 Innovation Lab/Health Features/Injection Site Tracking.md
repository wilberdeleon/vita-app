...
**Sprint 5 slice 5.5 (2026-09-05, awaiting founder device review) made the tool a weekly history.** The shared `BodyMap` gained an optional marker layer — one violet mark per place used in the selected week, carrying the weekday for a single use and a **count** when a place was used more than once, because stacked circles are unreadable and untappable. Monday-to-Sunday, with previous/next week. **Everything the figure shows is also written out beneath it** as rows that open their own logs, because a drawing is not an accessible interface on its own. The all-time list the tool had before is kept, disclosed underneath — narrowing the default view is not the same as removing history — and tapping a zone still reports when it was last used.

**The line is unchanged and was re-pinned by tests.** No recommended next site, no avoid, no rotate-here, no rest periods, no spacing advice, no scoring. A parameterised test sweeps Routine, Edit Routine and Injection Sites for fourteen forbidden phrases and for any percentage. The routine screen shows a **small** contextual list of the week's sites — only when that routine has site-tagged logs — and hands the full picture to this tool rather than drawing a second figure.
