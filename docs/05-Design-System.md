...
Existing primitives (in `src/components/ui/`): Screen, ScreenHeader, Card, GlassSurface, Section, SectionHeader, ProgressBar, AccentRail, ProgressRing, StatBar, DailyProgressCard, SegmentedTabs, Chip, ListRow, IconBadge, Button, TextField, NumericField, Stepper, PressableScale, EmptyState, Toast, VitaSheet. *(`PressableCard` was removed in slice 5.1 — zero call sites.)*
...
**The primary action is neutral, and it names what it does.** Restated 5.6D as a standing rule after Food Detail shipped a full-width orange slab for three slices: the primary action takes the theme's own high-contrast fill — white on the near-black page, brand ink on the cream one — and the feature colour stays on the objects and states around it. `Button variant="neutral"` is that treatment. Where the action commits something to a named place, it says so: **`Add to Dinner`**, not `+ Add to Log`, so the last thing a person reads before acting is the thing they might have got wrong.

**And there is a second weight: outlined.** Amended 2026-09-15 by founder ruling on the Peptides first-use screen. `variant="neutral"` is a solid high-contrast block, which is correct for the commit at the end of a flow and **too loud as the only object on an otherwise-empty page** — a white slab in place of a violet one, which was the founder's rejection. `Button variant="outline"` is the same action one step quieter: a hairline border in `surfaces.border`, `surfaces.card` inside it, the label at full contrast, and the feature colour spent on the glyph alone.

It is not a new style. It is the treatment already drawn by hand at roughly **eight call sites** — Fuel Home's `Add food`, Home's `Log` and `Add` pills, Fuel's `Set up Fuel`, Water's `Set goal`, a routine's `Taken` and `Skipped` — and the only one of VITA's action languages that had no shared component. The eight remain hand-drawn for now; migrating them to the shared variant belongs to **5.7**.
...
**A rail that shows progress is tinted, not grey.** Amended 2026-09-15, same pass. `surfaces.track` belongs to steppers, segmented controls and inert wells; under a coloured fill it reads as a dead grey sliver, which was the founder's word for it. `AccentRail` — one shared component, used by Fuel Home's calorie rail and Home's Fuel widget — draws the track as a **low-opacity tint of the fill**, the same treatment VITA already uses for the flame's orb and the Peptides mark. The rail then reads as one object at two intensities and the seam where fill meets track disappears. 4pt, pill-capped, no glow and no gradient. `ProgressBar` keeps `surfaces.track` as its default, so every other bar is unchanged.

**Macro colour is category identity, at every scope.** `macroAccent` is the single source for Protein, Carbs and Fat wherever they appear — Fuel Home's daily totals, an individual food's detail, the manual form's fields. The accent sits on the label; the figure stays neutral and high-contrast. `palette.protein` / `.carbs` / `.fat` are green, amber and red, are a traffic light side by side, and **are not the macro colours** — they remain only as legacy tokens (`palette.fat` is also the app's destructive red). **Item-level nutrition never carries a goal**: no target, no remainder, no percentage, no rail, no denominator. A food is not measured against a person's day.
